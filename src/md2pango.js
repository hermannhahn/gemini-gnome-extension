...
const re_color =
    /^(\s*<!--\s*(fg|bg)=(#?[0-9a-z_A-Z-]*)\s*((fg|bg)=(#?[0-9a-z_A-Z-]*))?\s*-->\s*)$/;
const re_reset = /(<!--\/-->)/;
const re_uri = /http[s]?:\/\/[^\s']*/;
const re_href = "/href='(http[s]?:\\/\\/[^\\s]*)'";
const re_atag = '<as.*>.*(http[s]?:\\/\\/[^\\s]*).*</a>/';
const re_h1line = /^===+\s*$/;
const re_h2line = /^---+\s*$/;
...
const escape_line = (line) =>
    m2p_escapes.reduce((l, esc) => l.replace(...esc), line);

const pad = (lines, start = 1, end = 1) => {
    let len = lines.reduce((n, l) => (l.length > n ? l.length : n), 0);
    return lines.map((l) =>
        l.padEnd(len + end, ' ').padStart(len + end + start, ' '),
    );
};
